...
// This filter acts before Delphes processing
{
  ...
  jetDef = new fastjet::JetDefinition(fastjet::antikt_algorithm, 0.8, recombScheme, strategy);
  ...
  // check if the leading jet has pT > 150
  if (sortedJets.size() == 0 || sortedJets[0].pt() <= 150) {
    return false;
  }
