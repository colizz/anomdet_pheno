...
// This filter acts after Delphes processing
{
  ...
  for(int i = 0; i < fatjets->GetEntries(); ++i)
  {
    ...
    if (candidate->Momentum.Pt() > 200 && std::abs(candidate->Momentum.Eta()) < 2.5) {
      // std::cout << i << " " << candidate->Momentum.Pt() << " GeV/c and eta = " << candidate->Momentum.Eta() << std::endl;
      fjHT += candidate->Momentum.Pt();
      if (leadfjMass == 0) {
        leadfjMass = candidate->TrimmedP4[0].M();
      }
    }
  }
  ...
  // std::cout << "//" << pass << "// fjHT = " << fjHT << ", maxMass = " << maxMass << std::endl;
