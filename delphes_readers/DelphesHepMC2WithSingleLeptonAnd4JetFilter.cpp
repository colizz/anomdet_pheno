...
{
  // cluster genjets (AK4) from stable particles
  fastjet::Strategy               strategy = fastjet::Best;
  fastjet::RecombinationScheme    recombScheme = fastjet::E_scheme;
  fastjet::JetDefinition         *jetDef = NULL;
  jetDef = new fastjet::JetDefinition(fastjet::antikt_algorithm, 0.4, recombScheme, strategy);
  ...
  // extract inclusive jets sorted by pT (note minimum pT of 18.0 GeV)
  std::vector<fastjet::PseudoJet> inclusiveJets = clustSeq.inclusive_jets(18.0);
  std::vector<fastjet::PseudoJet> sortedJets = sorted_by_pt(inclusiveJets);

  // check if there are >=4 jets with pT > 18 GeV
...
bool pass4JetFilter(Delphes *modularDelphes)
{
  bool pass = false;
  TObjArray *jets = modularDelphes->ImportArray("JetEnergyScalePUPPI/jets");

  // requires at least 4 jets with pT > 20
  if (jets->GetEntries() >= 4) {
    Candidate *j4 = static_cast<Candidate *>(jets->At(3));
    if (j4->Momentum.Pt() > 20) {
      pass = true;
    }
  }
  return pass;
...
{
  return ((passIsoEle32Filter(modularDelphes) || passIsoMu24Filter(modularDelphes)) && pass4JetFilter(modularDelphes));
}
