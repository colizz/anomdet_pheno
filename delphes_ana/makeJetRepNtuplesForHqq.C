...
void makeJetRepNtuplesForHqq(TString inputFile, TString outputFile, TString modelPath, TString jetBranch = "JetPUPPIAK8") {
  ...
  floatVars["part_h_pt"] = 0;
  floatVars["part_h_eta"] = 0;
  floatVars["part_h_phi"] = 0;
  floatVars["jet_dr_hdau1"] = 0;
  floatVars["jet_dr_hdau2"] = 0;
  ...
  for (Long64_t entry = 0; entry < allEntries; ++entry) {
    ...
    // select AK8 jet matched with H->qq

    // first, find the Higgs boson
    ...
    for (Int_t i = 0; i < branchJet->GetEntriesFast(); ++i) {
      ...
      if (!(deltaR(jet, hdaus.at(0)) < jetR && deltaR(jet, hdaus.at(1)) < jetR))
        continue;

      // std::cerr << "Find matched jets. Event = " << entry << ", jet pT = " << jet->PT << ", jet eta = " << jet->Eta << ", jet phi = " << jet->Phi << std::endl;
      ...
      floatVars["part_h_pt"] = higgs->PT;
      floatVars["part_h_eta"] = higgs->Eta;
      floatVars["part_h_phi"] = higgs->Phi;
      floatVars["jet_dr_hdau1"] = deltaR(jet, hdaus.at(0));
      floatVars["jet_dr_hdau2"] = deltaR(jet, hdaus.at(1));
